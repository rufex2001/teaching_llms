...
% header and footer
\lhead{Advanced Methods in Text Analytics, FSS 2026}
\chead{}
\rhead{\thepage\ }
\cfoot{}
\pagestyle{fancy}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item The likelihood in causal language modeling (CLM) can be defined as
          follows:
          \begin{align}
              \op{CLM}(S) & = \prod_{s=1}^{S} p(x_{|s|} \mid x_1, x_2, \ldots, x_{|s|-1}),
          \end{align}
          where $x_i$ are tokens in sequence $s$.
          Similarly, for masked language modeling (MLM), we have:
          \begin{align}
          ...
          We use these likelihoods in combination with empirical risk 
          minimization with log loss to train these models. That is,
          \begin{align}
